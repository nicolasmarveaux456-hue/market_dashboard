# Market Dashboard

## Objective

This project transforms the momentum backtesting framework developed in the Europe Momentum Backtest project into an interactive dashboard.

The objective is to provide a simple interface for exploring:

- market data,
- strategy performance,
- portfolio composition,
- risk metrics,
- robustness analysis.

The dashboard is built using Streamlit and Plotly.

---

## Main Features

### Market Data

- Historical price visualization
- Monthly returns visualization
- Interactive ticker selection

### Strategy Analytics

- Momentum strategy performance
- FEZ benchmark comparison
- Performance metrics

### Risk Analysis

- Drawdown analysis
- Rolling volatility
- Turnover monitoring
- Gross vs net performance

### Robustness Analysis

- In-sample / out-of-sample results
- Parameter robustness heatmap
- Portfolio exploration

---

## Project Structure

```text
market_dashboard/
│
├── app/
│   └── dashboard.py
│
├── data/
│
├── notebooks/
│
├── src/
│   ├── data_loader.py
│   └── charts.py
│
├── README.md
└── requirements.txt